...
func main() {
	...
	for _, host := range flag.Args() {
		...
		for cipherName, cipherID := range cipherSuitesIANA {
			if verbose {
				fmt.Printf("%s (0x%04x): ", cipherName, cipherID)
			}
			...
			if err == nil {
				...
				supported = append(supported, cipherName)
			} else if verbose {
				fmt.Printf("FAILED (%v)\n", err)
			}
		}
		fmt.Printf("%d ciphersuites supoorted: \n", len(supported))
		for _, cipherName := range supported {
			fmt.Printf("\t0x%04x: %s\n", cipherSuitesIANA[cipherName], cipherName)
		}
	}
}
...
var cipherSuitesIANA = map[string]uint16{
